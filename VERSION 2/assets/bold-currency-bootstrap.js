if (
  typeof BOLDCURRENCY !== "undefined" &&
  BOLDCURRENCY.converter &&
  typeof BOLD_mc_bootstrapSettings !== "undefined"
) {
...
function BOLD_mc_bootstrap(options) {
  ...
  BOLDCURRENCY.converter.requestRates = function(callback, moneyElements) {
    BOLD.helpers.js
      .get(options.shopDomainUrl + "/rates")
  ...
  BOLDCURRENCY.converter.postCartDataToCashier = function(
    cartId,
    callback,
    moneyElements
  ) {
    BOLD.helpers.js
      .post(options.shopDomainUrl + "/rates", { cart_id: cartId })
      .then(function(returnedValue) {
        if (returnedValue.hasOwnProperty("error")) {
          BOLDCURRENCY.enabled = false;
          console.info("Bold MultiCurrency: " + returnedValue.error);
    ...
    BOLD.helpers.setupMutationObservers = function(target, eventToEmit) {
      var observer = new MutationObserver(function(mutations) {
        mutations.forEach(function(mutation) {
          ...
          BOLD.helpers.shopify
            .getVariant(
              BOLD.helpers.shopify.getProductHandleById(variantId),
              variantId
            )
            .then(function(data) {
  ...
  document.addEventListener("DOMContentLoaded", function(event) {
    if (window.jQuery && jQuery().on) {
      jQuery(".bold_product_page_price")
        .parent()
        .each(function(i, parent) {
          BOLD.helpers.setupMutationObservers(
            parent,
            "BOLD_CURRENCY_product_price_updated"
          );
        });
    }
  ...
  function initPickers() {
    var pickerTemplateParent = document.getElementById(
      "bold-currency-picker-template"
    );
    var mountpointTemplateParent = document.getElementById(
      "bold-currency-picker-mount-template"
    );
    ...
    var mountpointInstances = document.getElementsByClassName(
      "BOLD-mc-picker-mnt"
    );
    if (0 === mountpointInstances.length) {
      // inject automatically if necessary
      var injectedMount = document.body.appendChild(
        getElementFromTemplate(mountpointTemplateParent)
      );
    ...
    Array.prototype.forEach.call(mountpointInstances, function(instance) {
      var appendedChild = instance.appendChild(
        getElementFromTemplate(pickerTemplateParent)
      );
